...
archivoSecEmitidos="secuenciaArchivosEmitidos"
archivoSecDuplicados="secuenciaArchivosRechazadosDuplicados"
archivoRechazados="$recha/Plasticos_rechazados.txt"
archivoCumae="$MAESTROS/cumae"
archivoBamae="$MAESTROS/bamae"
archivoTarjetas="$MAESTROS/tx_tarjetas"
...
#Chequea las validaciones del registro dado
	...
	if [ $? -eq 0 ]; then
		motivo="Fecha Desde incorrecta"
		return 0
	fi
	validarFecha $regFechaHasta
	if [ $? -eq 0 ]; then
		motivo="Fecha Hasta incorrecta"
		return 0
	fi
